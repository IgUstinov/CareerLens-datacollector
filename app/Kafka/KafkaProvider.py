...
class KafkaProvider:
    ...
    async def start_collect_data(self, msg):
        ...
        async for batch in self.collector.get_vacancies_batches(collectData['count']):
            ...
            added = {"Ids": result, "requestId":requestId}
    ...
    async def process_message(self, message):
        try:
            ...
            requestId = message.get("requestId")  # Предполагаем, что requestId приходит с запросом
            ...
            if filtered_data:
                ...
                response = {"collectionName": new_collection_name, "requestId": requestId}
    ...
    async def flatten_dict(self, data, parent_key='', separator='.'):
        ...
        for k, v in data.items():
            if(k=='salary'):
                items['$or'] = [
                        {'salary.from': {'$gte': v['value']}},  # from больше или равно указанного значения
                        {'salary.to': {'$lt': v['value']}}  # to меньше указанного значения
                    ]
                continue
    ...
    async def CountryRequest(self, msg):
        ...
        logging.info("Add data: ", result)
        await self.added_data_to_mongodb({"Ids": result, "requestId": requestId})
